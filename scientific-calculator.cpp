...
//funkce 
void solver(char u_input[128]);
Token ConvertToToken(char input_c);
void AssignStack(Token token);
void PostFixEvaluator();
void PrintResult(double r, double i,char *result);
void SaveResult(char *result,char path[]);
void LoadResults(bool silent,char path[]);
Token ResultPicker(int id);
void Commands(char input);
void FileSolver(char* inputPath, char* outputPath);
void ClearStacks();
...
bool repeat = "az naprsi a uschne";
char savedPath[] = "saved";
...
void solver(char u_input[128]) {	// Přepíše vstup pomocí shunting yard algoritmu    https://en.wikipedia.org/wiki/Shunting_yard_algorithm
	char numUnify[1024];
	int numCount = 0;
	char *ptr = u_input;
	if (*ptr == '?') {
	...
	while(*ptr != '\0' || numCount != 0) {
		...
		if (numCount == 0 && *ptr != '\0') { // pro operandy
...
void PrintResult(double r, double i,char *result) {
	while (false == false) {
		if ((int)(round(r)) == 0) {
			snprintf(result,sizeof(result),"%.4lfi\0", i);
			break;
		}
		if ((int)(round(i)) == 0) {
			snprintf(result, sizeof(result), "%.4lf\0", r);
			break;
		}
		i < 0 ? snprintf(result, 256, "%.4lf%.4lfi\0", r, i) : snprintf(result, 256, "%.4lf+%.4lfi\0", r, i);
		break;
	}
}
...
void LoadResults(bool silent,char path[]) {
	...
	while (c != EOF) {
		if ((char)(c) == ';') {
			...
			newResult->id = id;
			newResult->next = NULL;
			strcpy_s(newResult->result, 256, buffer);
			if (firstResult == NULL) {
...
Token ResultPicker(int id) {
	Token token{ 0,'n',complex{0,0},0,NULL };
	struct t_result* aktResult;
	if (firstResult == NULL) {
		LoadResults(true,savedPath);
	}if (firstResult == NULL) {
